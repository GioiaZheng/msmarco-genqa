...
| `subprocess` with `shell=True` | ✅ none | All seven `subprocess` call sites pass argv lists, not shell strings. Sites: `src/reporting/build_report.py` (pandoc PDF render), `src/util/manifest.py` (`git rev-parse`, `git status`), `src/util/environment.py` (`git rev-parse`), `tests/test_sampling.py` (test fixtures), `scripts/run_full_generation_and_analysis.py` (driver subprocess). All commands are fixed strings + Path values, not user-string-interpolated. |
