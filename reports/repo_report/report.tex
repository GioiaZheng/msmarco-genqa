...
appears only at ranks 101--1,000.  The evidence separates candidate
absence, depth recoverability, and dataset/query representation
effects; it does not by itself justify a new retrieval architecture.
...
The only complete manual taxonomy currently covers the 72 NFCorpus
no-hit-at-100 cases; 67 of them are labelled
\texttt{source\_context\_dependency}.  SciFact has the matched
quantitative diagnostic but not a matched manual taxonomy census.  The
defensible conclusion is therefore narrow: NFCorpus has a severe
candidate-set and compact-query problem, SciFact generalizes better at
the first stage, and the cross-encoder cannot recover evidence missing
from the fixed \bmtwofive{} candidate set.  The command
...
\paragraph{The cross-dataset first-stage analysis is still quantitative.}
...
two failure partitions directly.  This does not explain every residual
miss.  The 24 SciFact depth-recoverable and 11 top-1,000-miss queries
still require case-level inspection before attributing them to
vocabulary mismatch, claim wording, qrels coverage, or model capacity.
Only the NFCorpus no-hit-at-100 census has a complete manual taxonomy.
...
NFCorpus by $+0.6381$ \recallH{} and by $+80.5$ percentage points in
complete top-100 coverage share.  The large NFCorpus ceiling should
therefore be treated as dataset- and representation-sensitive rather
than a general failure of the unchanged first stage.
