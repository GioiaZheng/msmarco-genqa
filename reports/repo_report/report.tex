...
scanning, a deterministic RAG triad CLI, and a clearer advanced-RAG
roadmap.
...
\begin{itemize}
  \item \textbf{Workflow facade.}  \texttt{rag-eval run --config
        ...
        retrieval, reranking, generation, bootstrap, grounding, and
        triad-evaluation workflow from one entry point.
...
post-hoc analysis layers (W6--W8) in implementation order.  All
of W2--W5 ship as scripted command-line entrypoints under
\texttt{experiments/run\_*.py}, driven by a single configuration
file at \texttt{configs/baseline.yaml}; W6--W7 are analysis scripts
under \texttt{scripts/*.py}, and W8 is an installable
\texttt{mgq-rag-triad} diagnostic that reads W3's already-on-disk
paired predictions and writes JSON / Markdown summaries beneath
\texttt{outputs/W6\_*/}, \texttt{outputs/W7\_*/}, and
\texttt{outputs/W8\_rag\_triad/}.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[aux, below=10mm of gen]             (eval)
      {\textbf{W6}~semantic proxy\\\textbf{W7}~grounding audit\\\textbf{W8}~triad report};
...
\subsection{W8 --- RAG triad diagnostic interface}
\label{sec:w8-triad}

The current repository adds a deterministic triad report as an
engineering-facing diagnostic layer over the same W3 prediction
...
\begin{itemize}
  ...
  \item \textbf{Evaluation hardening.}  Extend the current paired
        ...
        correction, and TREC-DL qrels loaders.
...
at \texttt{configs/baseline.yaml}; the W6/W7 analysis scripts and
W8 triad CLI read the on-disk W3 paired predictions directly.  The current repository
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{l>{\ttfamily\raggedright\arraybackslash}X}
    ...
    W8 triad     & mgq-rag-triad \mbox{--predictions} bm25=\ldots{} \mbox{--predictions} reranked=\ldots{} \mbox{--output-dir} outputs/W8\_rag\_triad \\
...
\begin{itemize}
  ...
  \item \textbf{W8 triad:} deterministic context relevance,
        groundedness, and answer relevance diagnostics over one or
        more generation prediction files.  Output:
        \code{outputs/W8\_\allowbreak rag\_\allowbreak triad/}.
\end{itemize}
\endgroup
