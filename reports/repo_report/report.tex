...
\begin{titlepage}
  ...
  {\renewcommand{\arraystretch}{1.30}%
   \begin{tabular}{@{}r@{\hskip 1.4em}l@{}}
     ...
     \textbf{First-stage analysis}    & 2026-08-11 \quad{\small(NFCorpus / SciFact / cross-dataset diagnosis)} \\
     \textbf{Current update}          & 2026-08-11 \\
...
This 2026-08-11 update keeps the empirical claims anchored to the
...
The matching cross-dataset error analysis shows why the pipeline should
remain frozen for the next interpretation step.  SciFact has much
healthier candidate coverage: \recallH{} is 0.8759 rather than 0.2378,
...
mismatches, 4/35 as lexical competition at the depth cutoff, and 3/35
as short or broad claims.  The evidence separates candidate absence,
depth recoverability, and dataset/query representation effects; it
does not by itself justify a new retrieval architecture.
...
\begin{itemize}
  ...
  \item \textbf{SciFact first-stage diagnosis.}  The matching
        ...
        have complete relevant-document coverage at depth 100.  The
        residual 35-query review points mostly to scientific
        claim/evidence formulation mismatches, keeping the NFCorpus
        source-context ceiling from being overgeneralised across all
        BEIR evidence.
  \item \textbf{Cross-dataset first-stage error analysis.}  The
        combined NFCorpus/SciFact diagnosis separates candidate-set
        absence, depth-recoverable misses, residual top-1,000 misses,
        and query/dataset effects.  It supports keeping the pipeline
        unchanged while deciding whether a later retrieval-side change
...
\begin{table}[H]
  ...
  \input{../generated/tables/trec_dl_2019_bm25_ce.tex}
\end{table}
...
\begin{table}[H]
  ...
  \input{../generated/tables/trec_dl_2020_bm25_ce.tex}
\end{table}
...
\begin{table}[H]
  ...
  \input{../generated/tables/beir_cross_domain_bm25_ce.tex}
\end{table}
...
NFCorpus relevant documents never reach the reranker.  The next
interpretation step is to separate candidate-set absence, candidate
depth, and query/dataset representation effects before selecting a
retrieval-side change.  This is a two-dataset retrieval-transfer
result, not a claim that the complete RAG pipeline or its generator
...
the same scale.  The follow-up question is therefore not whether to
change the reranker, but why the remaining 35 fixed-candidate misses
occur.

\subsection{SciFact residual first-stage failure review}
\label{sec:scifact-residual-review}

The residual review covers exactly those 35 SciFact queries with no
judged relevant document in the \bmtwofive{} top 100: 24 whose first
positive appears only at ranks 101--1,000 and 11 with no positive even
at depth 1,000.  It reuses the frozen \bmtwofive{} depth-1,000 run,
public qrels, SciFact claims, and corpus abstracts.  No retrieval,
reranking, or generation is rerun.

\begin{table}[H]
  \centering\small
  \caption{Bounded review of the 35 residual SciFact no-hit-at-100 cases.}
  \label{tab:scifact-residual-review}
  \begin{tabularx}{\linewidth}{Xrr}
    \toprule
    Primary description & Queries & Share \\
    \midrule
    Terminology/evidence-form mismatch & 28 & 80.0\% \\
    Lexical competition at the depth cutoff & 4 & 11.4\% \\
    Short or broad claim & 3 & 8.6\% \\
    Other unclear & 0 & 0.0\% \\
    \bottomrule
  \end{tabularx}
\end{table}

The cross-cutting signals are consistent with this interpretation.  In
all 35 cases, the strongest \bmtwofive{} candidates match more claim
tokens than the judged positive evidence.  In 31/35 cases, the judged
positive evidence covers at most 25\,\% of the claim content tokens;
13/35 contain negation, direction, activation, inhibition, or
comparative cues; and 10 queries share one of five judged-positive
evidence documents with another residual claim.

This does not reproduce the NFCorpus source-context pattern.  The
SciFact residual tail is better described as scientific claim/evidence
formulation under exact lexical matching and a fixed top-100 reranker
cutoff.  The pipeline therefore remains frozen for the report.  A
future change should be predeclared and retrieval-side only if it
targets this observed failure mode, for example candidate-depth
sensitivity, scientific-claim query rewriting, or hybrid lexical/dense
retrieval.

\subsection{Cross-dataset first-stage error analysis}
\label{sec:cross-dataset-errors}

The two first-stage diagnostics can be compared directly because they
use the same frozen BEIR release bundle, the same binary relevance rule
(\texttt{rel >= 1}), the same \bmtwofive{} depth of 1,000, and the same
top-100 reranker reachability cutoff.  The comparison is retrieval-only:
it reads the already-published ranked outputs and does not rerun
retrieval, reranking, or generation.

\begin{table}[H]
  \centering\small
  \caption{Cross-dataset first-stage error partition under the fixed
  \bmtwofive{} top-100 reranker condition.}
  \label{tab:cross-dataset-errors}
  \begin{tabularx}{\linewidth}{Xrr}
    \toprule
    Partition & NFCorpus & SciFact \\
    \midrule
    No relevant document in top 100 & 72 (22.3\%) & 35 (11.7\%) \\
    First relevant hit only at ranks 101--1,000 & 24 (7.4\%) & 24 (8.0\%) \\
    No relevant hit at depth 1,000 & 48 (14.9\%) & 11 (3.7\%) \\
    Partial top-100 coverage & 232 (71.8\%) & 6 (2.0\%) \\
    Complete top-100 coverage & 19 (5.9\%) & 259 (86.3\%) \\
    \bottomrule
  \end{tabularx}
\end{table}

The headline gap is not just query-level no-hit rate.  SciFact exceeds
NFCorpus by $+0.6381$ \recallH{} and by $+80.5$ percentage points in
complete top-100 coverage share.  NFCorpus is dominated by partial
coverage: at least one relevant document is often reachable, but most
positive qrels remain outside the candidate set.  SciFact, in contrast,
usually gives the cross-encoder all judged positive evidence within the
fixed candidate pool.

The complete NFCorpus no-hit-at-100 census labels 67/72 cases as
\texttt{source\_context\_dependency}.  The bounded SciFact residual
review instead labels 28/35 cases as terminology/evidence-form
mismatches, 4/35 as lexical competition at the depth cutoff, and 3/35
as short or broad claims.  The defensible conclusion is therefore
narrow: NFCorpus has a severe candidate-set and compact-query problem,
SciFact generalizes better at the first stage, the residual SciFact
misses have a different formulation-driven shape, and the
cross-encoder cannot recover evidence missing from the fixed
\bmtwofive{} candidate set.  The command
\texttt{make analyze-cross-dataset-errors} verifies the two source
summaries against \texttt{configs/cross\_dataset\_error\_analysis.json}
and writes a machine-readable comparison under
\texttt{outputs/analysis/cross\_dataset\_errors/}.
...
\begin{table}[H]
  ...
  \input{../generated/tables/nfcorpus_video_query_representation_bm25.tex}
\end{table}
...
\begin{table}[H]
  ...
  \input{../generated/tables/nfcorpus_video_query_representation_bm25_ce.tex}
\end{table}
...
\paragraph{The cross-dataset first-stage analysis remains bounded.}
...
\bmtwofive{} run, and \Cref{sec:cross-dataset-errors} compares the
two failure partitions directly.  \Cref{sec:scifact-residual-review}
adds a bounded review of the 35 SciFact no-hit-at-100 cases, but the
labels are descriptive and token-overlap aided rather than causal
ground truth.  They cover the residual miss set only, not all 300
SciFact queries, and they do not evaluate generated answers or
groundedness on SciFact.
...
complete relevant-document coverage at depth 100.  The combined
cross-dataset partition shows the sharper contrast: SciFact exceeds
NFCorpus by $+0.6381$ \recallH{} and by $+80.5$ percentage points in
complete top-100 coverage share.  The residual SciFact review further
shows that the remaining 35 no-hit-at-100 cases are mostly
claim/evidence formulation mismatches rather than NFCorpus-style
source-context failures.  The large NFCorpus ceiling should therefore
be treated as dataset- and representation-sensitive rather than a
general failure of the unchanged first stage.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{l>{\ttfamily\raggedright\arraybackslash}X}
    ...
    SciFact residual review & make review-scifact-first-stage \\
    Cross-dataset error analysis & make analyze-cross-dataset-errors \\
