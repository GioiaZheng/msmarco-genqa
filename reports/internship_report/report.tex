...
\hypersetup{
  ...
  pdfsubject={Internship Report --- 11-week project (12-week plan)},
...
\begin{titlepage}
  ...
  {\renewcommand{\arraystretch}{1.30}%
   \begin{tabular}{@{}r@{\hskip 1.4em}l@{}}
     \toprule
     \textbf{Role}    & NLP Algorithm Engineer Intern \\
     \textbf{Mentor}  & Tyler \\
     \textbf{Period}  & 2026-03-06\,--\,2026-05-21 \quad{\small(11 weeks elapsed; 12-week plan)} \\
     \textbf{Scope}   & Single-machine, CPU-only; English passage retrieval + QA \\
     \textbf{Date}    & 2026-05-21 \\
     \bottomrule
   \end{tabular}\par}
  ...
  % --- Repository footer ----------------------------------------------------
  {\small\textsc{Project repository}\\[0.15em]
   \href{https://github.com/GioiaZheng/msmarco-genqa}%
    {\texttt{github.com/GioiaZheng/msmarco-genqa}}\par}
...
The question-form breakdown is reported inline with the rest of W6
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[stage, right=8mm of q]              (fs)  {First stage\\\textbf{W2}~BM25\\\textbf{W4}~Dense};
    \node[stage, right=8mm of fs]             (ce)  {\textbf{W5}\\CE rerank};
    \node[stage, right=8mm of ce]             (gen) {\textbf{W3}\\T5-small RAG};
    ...
    \node[aux, below=10mm of gen]             (eval)
      {\textbf{W6}~semantic proxy\\\textbf{W7}~grounding audit};
  ...
  evaluation layer (\Cref{sec:w6,sec:w7}), which consumes the
  already-on-disk W3 paired predictions and does not re-run any
  modelling stage.  W2 and W4 are interchangeable first stages
  feeding into the same downstream pipeline; the W3 paired
...
\subsection{W4-B: encoder horizontal on the W4 sample}
\label{sec:w4b}

\Cref{tab:w4b-encoders} compares three same-tier general-purpose
encoders on the identical W4 50k qrels-anchored sample.  All three
...
\begin{table}[H]
  \centering
  \caption{W4-B --- same-tier dense encoders on the W4 50k
  ...
  \begin{tabular}{lrrrrr}
    ...
    \midrule
    \texttt{all-MiniLM-L6-v2} (W4 baseline) & 0.8830 & 0.9041 & 0.9946 & 0.9991 & \textbf{16.2} \\
...
the W4 baseline at $\sim\!2\!\times$ encoding cost (CPU,
...
none of the three encoders dominates strictly.  The W4-A density
sweep (\Cref{sec:w4a}) uses bge-small as the encoder.

\subsection{W4-A: density sensitivity of the dense vs.\ \bmtwofive{} gap}
\label{sec:w4a}

The W4 50k baseline is one point on a wider trade-off:
\emph{how does the dense-vs-\bmtwofive{} gap shift as the sample
dilutes?}  \Cref{tab:w4a-density} sweeps three sample sizes (15k,
30k, 50k) with bge-small (W4-B winner) and the same BM25-on-sample
baseline at each size.

\begin{table}[H]
  \centering
  \caption{W4-A --- density sensitivity of the same-sample
  BM25-vs-dense gap. Encoder: \texttt{BAAI/bge-small-en-v1.5}
  (W4-B winner). Density is the share of the sample that is
...
\to +0.207$.  This is consistent with the literature claim that
\bmtwofive{} is competitive at high relevant-density and loses
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[anchor=east, font=\small] at (-0.2, 1) {\Tokenf{}};
    \draw[bar] (0.1632*\sx, 1) -- (0.1789*\sx, 1);
    \node[point] at (0.1711*\sx, 1) {};
    \node[anchor=west, font=\scriptsize, gray!60!black]
      at (0.1789*\sx + 0.15, 1) {$+0.171$};
  \end{tikzpicture}
  \caption{Headline W3 paired-bootstrap forest plot
...
Consolidating the deferred items recorded throughout the report
(\Cref{sec:plan-vs-actual}), the most immediately tractable
follow-ups, in roughly descending expected information-gain order:

\begin{itemize}
  \item \textbf{W7-B --- T5-base generator swap.}  The disambiguating
        experiment of \Cref{sec:discussion-nli-flip}: rerun the W3
        paired comparison with T5-base (frozen, same prompt) and
        recompute both the surface-form $\Delta$ and the NLI
        grounding $\Delta$.  Resolves whether the W7-A sign flip
        is an output-shape artefact or a genuine semantic
        regression.
  \item \textbf{W4-B propagation through W5\barrow{}W3.}  Propagate
        the BGE-small-en-v1.5 encoder (\Cref{tab:w4b-encoders})
        through the cross-encoder reranker and the generator and
        re-run the paired generation comparison.  Tests whether the
        $+0.019$ \mrr{} W4-B encoder gain survives the
        \recallH{}-saturated rerank step into a downstream
        \Tokenf{} $\Delta$.
  \item \textbf{W5-B --- rerank-depth $K$ sweep.}  $K\!\in\!\{50,
        100, 200\}$ with the same checkpoint, on a query-id
        subsample sized for the project's CPU budget.  Quantifies
        the marginal $\Delta$\mrr{} per additional reranked
        candidate.
  \item \textbf{Citation-grade BERTScore.}  Replace the W6
        DistilBERT proxy with \texttt{roberta-large} \bertscore{}
        on the full \fdev{} paired qid set.  Refines the absolute
        level number; the W6 proxy already pins direction and
        magnitude.
  \item \textbf{ANCE / msmarco-tuned dense encoder} as a strict
        upper-bound check on what \msmarco{}-specific tuning buys
        on top of the generic encoders already tried.
  \item \textbf{Long-document handling (W8).}  The sliding-window
        prompt assembly exists in the W3 runner; the missing piece
        is a benchmark pass on a long-doc slice.
  \item \textbf{Prompt-format ablations.}  Citation-aware /
        multi-passage-synthesis prompts to test whether the
        $\sim\!0.997$ lexical-grounding ceiling (\Cref{tab:w7-lex-ngram})
        can be moved off the floor of ``extract from a single
        passage''.
\end{itemize}

The reproducibility scaffold (\Cref{sec:reproducibility}) is
designed precisely so that each of these is one configuration edit
and one runner invocation, not a re-implementation.
