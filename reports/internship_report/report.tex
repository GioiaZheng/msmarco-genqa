% =============================================================================
% MS MARCO Generative QA — Internship Report
...
\titleformat{\section}{\Large\bfseries\sffamily}{\thesection}{0.6em}{#1}
\titleformat{\subsection}{\large\bfseries\sffamily}{\thesubsection}{0.55em}{#1}
\titleformat{\subsubsection}{\normalsize\bfseries\sffamily}{\thesubsubsection}{0.5em}{#1}
...
\fancyhead[RE,RO]{\small\itshape MS~MARCO Generative QA --- Internship Report}
...
\definecolor{codekey}{HTML}{0F62FE}
\definecolor{codecmt}{HTML}{6A737D}
\definecolor{codestr}{HTML}{0A7F32}
\usepackage{listings}
\lstset{
...
\begin{titlepage}
  \centering
  \vspace*{2.0cm}
  {\large\scshape NLP Algorithm Engineer Internship\par}
  \vspace{0.3em}
  {\normalsize\itshape Microsoft\par}
  \vspace{1.1cm}
  \rule{\linewidth}{0.6pt}\par
  \vspace{0.7cm}
  {\LARGE\bfseries MS~MARCO Generative QA:\\[0.3em]
   A Reproducible Retrieve--Rerank--Generate Pipeline\\[0.2em]
   with a Grounding Audit\par}
  \vspace{0.7cm}
  \rule{\linewidth}{0.6pt}\par
  \vspace{2.0cm}
  {\large GioiaZheng\\[0.2em]
   \href{mailto:gioia.zheng.stud@gmail.com}{gioia.zheng.stud@gmail.com}\par}
  \vspace{0.6cm}
  {\large Project repository:\\
   \href{https://github.com/GioiaZheng/msmarco-genqa}{\texttt{github.com/GioiaZheng/msmarco-genqa}}\par}
  \vfill
  \begin{center}
    \begin{tabular}{rl}
      Role    & NLP Algorithm Engineer Intern \\
      Mentor  & Tyler \\
      Date    & 2026-05-21 \\
      Period  & 2026-03-06 -- 2026-05-21 (11 weeks elapsed) \\
      Scope   & single-machine, CPU-only, English passage retrieval + QA \\
    \end{tabular}
  \end{center}
  \vspace*{1.3cm}
\end{titlepage}
...
This report documents an eleven-week internship project that
reproduces and analyses a complete \emph{retrieve--rerank--generate}
...
modelling stages, two layers of post-hoc evaluation (semantic proxy
\& grounding), an honest comparison between the initial 12-week
...
The internship brief was to build a complete
...
The brief named twelve weekly milestones spanning EDA, sparse
...
write-up.  In practice the project ran for eleven weeks on a
...
\begin{enumerate}
  \item \textbf{Honest baselines before optimisation.}  Every week
...
\begin{itemize}
  ...
  \item A reproducibility scaffold (per-run manifests with git
        ...
        number in this report from one command per week on a
...
stages (W1--W5) in implementation order.  \Cref{sec:results} is the
central empirical chapter, presenting the W2--W5 retrieval numbers,
the load-bearing W3 paired generation comparison, the W5-A
first-stage head-to-head, the W6 evaluation layer, and the W7
grounding audit.  \Cref{sec:discussion} discusses what the
combination of W6 closure and W7 grounding actually means about
...
Appendices cover the configuration, full bucket tables, the
question-form breakdown, and the reproducibility scaffolding.
...
the report; the K-sweep (\{50, 100, 200\}) is queued as W5-B
...
\begin{itemize}
  \item \textbf{\msmarco{} Passage Ranking} (the retrieval corpus): 8.8M
        ...
        retrieval baselines (W2 \bmtwofive{}, W4 dense, W5 rerank, and
        the W3 generator's upstream context) index this corpus.  Used
  ...
  \item \textbf{\msmarco{} Q\&A v2.1 validation split}: $\sim\!101$k
        ...
        more human-written reference answers.  W1 (EDA) uses the
        first 5\,000 rows of this split; W3 generation evaluation
...
length distributions on the W1 5\,000-row validation sample.
...
tokens), short enough that the W3 generator can fit three of them
into a T5-small 512-token input window without per-passage
truncation in the common case.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\linewidth]{query_length_distribution.png}
  \caption{Query-length distribution on the W1 5\,000-row \msmarco{}
  Q\&A v2.1 validation sample.  Median $\approx\!6$ tokens; the
  $\geq\!15$-token tail is small.  Sets the sizing target for the
  W3 RAG prompt budget.}
  \label{fig:query-length}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\linewidth]{passage_length_distribution.png}
  \caption{Passage-length distribution on the same W1 sample,
...
load-bearing W3 comparison uses, the proportions are
...
\Cref{fig:query-type,fig:answer-by-type} show the W1 distributional
...
\begin{enumerate}
  \item \texttt{DESCRIPTION} and \texttt{NUMERIC} dominate.
        ...
        class and motivates the W6 \bertscore{} proxy
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\linewidth]{query_type_distribution.png}
  \caption{\msmarco{} \texttt{query\_type} label distribution on the
  W1 validation sample.  \texttt{DESCRIPTION} dominates;
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\linewidth]{answer_type_by_query_type.png}
...
labelled.  This shapes the W4 sampling design (\Cref{sec:w4}): a
...
This section describes the four modelling stages (W2--W5) and the
two post-hoc analysis layers (W6--W7) in implementation order.  All
of W2--W5 ship as scripted command-line entrypoints under
\texttt{experiments/run\_*.py}, driven by a single configuration
file at \texttt{configs/baseline.yaml}; all of W6--W7 are analysis
scripts under \texttt{scripts/*.py} that read W3's already-on-disk
paired predictions and write JSON / Markdown summaries beneath
\texttt{outputs/week06\_*/} and \texttt{outputs/week07\_*/}.
\Cref{sec:reproducibility} documents the reproducibility scaffold.

\subsection{W2 --- Sparse retrieval (\bmtwofive{})}
\label{sec:w2}
...
$K\!\in\!\{100,1000\}$; \ndcg{} uses binary gain.

\subsection{W3 --- RAG generation baseline}
\label{sec:w3}
...
W6 closure run of \Cref{sec:w6-closure} sweeps.
...
The headline W3 comparison (\Cref{sec:gen-x-retrieval}) runs the
generator twice with exactly the same configuration, switching only
the upstream run between (i) the W2 \bmtwofive{} run and (ii) the W5
...
\subsection{W4 --- Dense retrieval (sampled)}
\label{sec:w4}

\paragraph{Encoder \& index.}  The W4 dense retriever uses
\texttt{sentence-transformers/all-MiniLM-L6-v2}~\cite{reimers2019sbert,wang2020minilm}
...
prohibitive (estimated $\sim\!40$~hours at the W4 encode throughput
of $16$ ms / passage).  The W4 baseline therefore runs on a
...
\Cref{tab:w4-results} reports.  Absolute W4 numbers are \emph{not}
directly comparable to the W2 full-corpus \bmtwofive{} number.
...
the W2 full-corpus baseline.

\subsection{W5 --- Cross-encoder reranking}
\label{sec:w5}
...
W4 bi-encoder which encodes them independently --- producing a
...
K-sweep ($\{50, 100, 200\}$) is queued as W5-B and explicitly
de-scoped per the project's reduced-scope deadline
(\Cref{sec:plan-vs-actual}).

\paragraph{Two arms.}  Two W5 runs are reported: one over the W4
dense top-100 (\Cref{sec:w5-dense-rerank}, the canonical setup) and
one over the W2 \bmtwofive{} top-100 (\Cref{sec:w5a}, the W5-A
head-to-head against the dense arm).  Both use the same reranker
checkpoint, the same depth, and the same evaluation pass.

\paragraph{Cost.}  The W5 dense+rerank full-dev run takes
$\approx\!4$~h~$37$~min on a 6-core MacBook CPU
(538\,000 (query, passage) pairs at $\sim\!32$~pairs/s, peak RSS
$\sim\!3.3$~GiB); the W5-A \bmtwofive{}+rerank arm takes
$\approx\!3$~h~$43$~min.  The cross-encoder is by far the slowest
stage in the pipeline.

\subsection{W6 --- Evaluation layer}
\label{sec:w6-method}

W6 layers two post-hoc analyses on the already-on-disk W3 paired
predictions, neither of which requires re-running W3 or W5:
...
\paragraph{W6-A/B/C breakdowns.}  Three offline analyses on the
...
\subsection{W7 --- Grounding audit}
\label{sec:w7-method}

W7 is the project's faithfulness layer.  The leading question is:
...
\paragraph{NLI-entailment grounding (W7-A).}  An NLI cross-encoder
(\texttt{cross-encoder/nli-deberta-v3-small}~\cite{he2023debertav3,williams2018mnli})
scores \emph{entailment}$(\text{passages} \to \text{prediction})$ on
the same 3\,000-paired-qid subsample used by the W6 \bertscore{}
...
produced: the per-stage retrieval baselines (W2 / W4 / W5,
...
$\times$ reranker head-to-head (W5-A, \Cref{sec:w5a}), and the
...
\subsection{W2: \bmtwofive{} baseline (full corpus)}
\label{sec:w2-results}

\Cref{tab:w2-results} reports the W2 \bmtwofive{} numbers on the full
8.8M-passage corpus and \fdev{} \devsmall{} queries, against the
published Anserini/Lucene reference.

\begin{table}[H]
  \centering
  \caption{W2 --- \bmtwofive{} on the full \msmarco{} Passage corpus,
...
\subsection{W4: Dense vs.\ \bmtwofive{} on the same sample}
\label{sec:w4-results}

\Cref{tab:w4-results} reports the W4 dense baseline against
\bmtwofive{} rebuilt on the \emph{same} qrels-anchored 50k sample.
Both columns evaluate on the same \fdev{} queries.

\begin{table}[H]
  \centering
  \caption{W4 --- dense vs.\ \bmtwofive{} on the same
...
\begin{enumerate}
  ...
  \item Dense \recallH{}~$=0.9946$ is effectively at the ceiling on
        this sampled setting.  From W5 onward, recall is no longer
...
\subsection{W5: Cross-encoder reranking on the dense top-100}
\label{sec:w5-dense-rerank}

\Cref{tab:w5-results} reports the W5 cross-encoder reranking of the
W4 dense top-100, on the full \fdev{} \devsmall{} queries.

\begin{table}[H]
  \centering
  \caption{W5 --- cross-encoder reranking of the W4 dense top-100,
  ...
  \begin{tabular}{lrrr}
    \toprule
    Metric        & Dense (W4) & Dense $+$ CE rerank & $\Delta$ \\
    \midrule
...
\begin{table}[H]
  \centering
  \caption{Headline W3 paired generation comparison on the full
...
\begin{table}[H]
  \centering
  \caption{W3 paired-bootstrap 95\,\% CI on $\Delta$ (rerank $-$
...
\begin{table}[H]
  \centering
  \caption{W3 paired \Tokenf{} broken down by \msmarco{}
...
\subsubsection{W4-B preliminary: encoder horizontal}
\label{sec:w4b}

Two follow-up encoder runs on the \emph{same} qrels-anchored 50k
sample, identical FAISS \texttt{IndexFlatIP} setup, evaluated on the
same \fdev{} queries (\Cref{tab:w4b}).  Both finished
inside the report's cutoff window; the BGE result in particular is
preliminary in the sense that it has not yet been propagated through
the W5 reranking and W3 generation stages.

\begin{table}[H]
  \centering
  \caption{W4-B preliminary --- encoder horizontal on the same
  qrels-anchored 50k sample, \fdev{} \devsmall{} queries.  The
  \bmtwofive{}-on-sample column is identical across rows (same
  index, same queries).  Numbers are upper-bounded by the
  qrels-anchored sampling design; the meaningful read is the
  same-sample $\Delta$ between encoders.}
  \label{tab:w4b}
  \begin{tabular}{lrrrr}
    \toprule
    Encoder                             & \mrr{} & \ndcg{} & \recallH{} & $\Delta\mrr{}$ vs.\ baseline \\
    \midrule
    \texttt{all-MiniLM-L6-v2} (W4)      & 0.8830 & 0.9041  & 0.9946 & ---                          \\
    \texttt{all-MiniLM-L12-v2}          & 0.8933 & 0.9131  & 0.9955 & $+0.0103$                    \\
    \texttt{BAAI/bge-small-en-v1.5}     & \textbf{0.9021} & \textbf{0.9196} & \textbf{0.9967} & $\mathbf{+0.0191}$ \\
    \bottomrule
  \end{tabular}
\end{table}

BGE-small-en-v1.5~\cite{xiao2024bge} is the best of the three
generic encoders tried so far on this sample, by $+0.019$ \mrr{}
over the W4 MiniLM-L6 baseline.  Whether the additional gain
survives cross-encoder reranking (the dense\barrow{}rerank pipeline
already saturates \recallH{}) is the W5\barrow{}W3 propagation
question and is left to the post-deadline follow-up.

\subsubsection{W4-A preliminary: density sensitivity}
\label{sec:w4a}

A complementary axis varies the pool size (relevant-doc density)
holding the encoder fixed at the W4 baseline.  \Cref{tab:w4a}
reports the 15\,000-sample point that finished before the report
cutoff; the 30\,000 and the full original 50\,000 are the other
two anchor points and the 30\,000 run is still encoding at write
time.

\begin{table}[H]
  \centering
  \caption{W4-A preliminary --- relevant-doc-density sensitivity
  with the W4 baseline encoder
  (\texttt{all-MiniLM-L6-v2}).  Same \fdev{} queries throughout;
  the qrels-anchored sample design keeps every dev relevant doc in
  the pool by construction at every sample size, so what changes
  across rows is the \emph{distractor density}, not the recall
  ceiling.}
  \label{tab:w4a}
  \begin{tabular}{lrrrr}
    \toprule
    Sample size      & Dense \mrr{} & Dense \ndcg{} & \bmtwofive{}-sample \mrr{} & $\Delta_{\mathrm{dense-BM25}}$ \\
    \midrule
    15\,000          & 0.9470 & 0.9567 & 0.7806 & $+0.1665$ \\
    50\,000 (W4)     & 0.8830 & 0.9041 & 0.6948 & $+0.1882$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  \item Absolute MRR drops monotonically as the pool grows, on
        both arms.  This is the expected direction --- more
        distractors per relevant doc.
  \item The \emph{gap} between dense and \bmtwofive{} widens with
        pool size ($+0.167$ at 15k, $+0.188$ at 50k).  Dense
        retrieval is more robust to distractor dilution than
        \bmtwofive{} is on this corpus.
\end{enumerate}

The 30\,000-sample run is in progress and is the next anchor
point; it should bisect the 15k\barrow{}50k gap and confirm
monotonicity.  A full $\{1\,000\,000, 8\,800\,000\}$ extension
(``what does the gap look like at the real corpus size?'') is
explicitly out of scope on this hardware budget.
...
\subsection{W5-A: First-stage $\times$ reranker head-to-head}
\label{sec:w5a}

A natural follow-up to \Cref{sec:w5-dense-rerank}: does the same
cross-encoder recover \emph{more} from a weaker first stage?
\Cref{tab:w5a-results} runs the identical reranker over the W2
\bmtwofive{} top-100 (full 8.8M corpus) and over the W4 dense
...
\begin{table}[H]
  \centering
  \caption{W5-A --- same cross-encoder applied to two first stages
...
question of W6 is therefore: does the rerank $\Delta$ also show up
...
\begin{table}[H]
  \centering
  \caption{W6 --- DistilBERT \bertscore{} proxy~\cite{zhang2020bertscore,sanh2019distilbert}
  on a 3\,000-pair subsample of the W3 \fdev{} shared qid set.
...
\begin{table}[H]
  \centering
  \caption{W6 --- regression failure taxonomy on a 40-query seeded
...
At the time of the W6 report this observation pointed at the
...
\begin{table}[H]
  \centering
  \caption{W6 decoding-budget closure --- same paired W3 comparison
...
\textbf{The W6 budget-cap hypothesis is falsified.}  T5-small is
...
\paragraph{W6-B --- rerank $\Delta$ by question-form.}
\Cref{tab:w6b} reports per-form $\Delta$ \Tokenf{} with
paired-bootstrap CIs.

\begin{table}[H]
  \centering
  \caption{W6-B --- rerank $\Delta$ \Tokenf{} by question-form on
...
\paragraph{W6-C --- structural profile of regression queries.}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.62\linewidth]{w6c_regression_vs_other_query_length.png}
  \caption{W6-C --- query-length distribution, regression bucket
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.62\linewidth]{w6c_regression_vs_other_n_qrels.png}
  \caption{W6-C --- number of relevance judgements per query.  A
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.62\linewidth]{w6c_regression_vs_other_top3_passage_length.png}
  \caption{W6-C --- average rerank top-3 passage length.  The only
...
The W6 closure (\Cref{sec:w6-closure}) ruled out the decode budget
as the bottleneck.  W7 answers the question the closure left open:
...
\begin{table}[H]
  \centering
  \caption{W7 --- lexical and 3-gram grounding on the full \fdev{}
...
\texttt{question:\ \ldots\ context:\ \ldots} prompt is performing
\emph{extractive} QA, not generative QA.  The W3 surface-form
...
\subsubsection{W7-A --- NLI-entailment grounding}
\label{sec:w7-nli}
...
on the same 3\,000-paired-qid subsample as the W6 \bertscore{}
proxy.

\begin{table}[H]
  \centering
  \caption{W7-A --- NLI-entailment grounding on the same 3\,000-
  paired-qid subsample (seed 42) used by the W6 \bertscore{} proxy.
  ...
  to the W3/W5/W6 surface-form story.}
  \label{tab:w7-nli}
  \begin{tabular}{lrrrrr}
    \toprule
...
\subsubsection{W7-C --- grounding $\leftrightarrow$ downstream
correlation}
...
\subsubsection{W7-D --- low-grounding case study}
...
\begin{table}[H]
  \centering
  \caption{W7-D --- low-grounding case study on a 30-query seeded
...
in the sample.  This reinforces the W7 headline: T5-small on this
...
is unambiguous: the headline doubling of W3 surface-form metrics is
...
axes.  \Cref{sec:plan-vs-actual} records the W7-B (T5-base swap)
follow-up that is queued in the project's reduced-scope plan.

\subsection{The W7-A NLI sign flip}
\label{sec:discussion-nli-flip}

\Cref{tab:w7-nli} is the only paired metric in this project whose
$\Delta$ reverses sign relative to the otherwise-consistent W3/W5/W6
...
The 40-query taxonomy (\Cref{tab:w6-taxonomy}) and the W7-D case
...
This is harder to rule out from the existing data because the W7
...
in W7-A is \texttt{cross-encoder/nli-deberta-v3-small} ---
...
swap: rerun the W3 paired comparison with T5-base (or an
...
working hypothesis.  This is the planned W7-B follow-up, deferred
to the post-deadline T5-base run (\Cref{sec:plan-vs-actual}).

\subsection{Plan vs.\ actual}
\label{sec:plan-vs-actual}

The project's initial 12-week plan named the milestones in
\Cref{tab:plan-vs-actual}.  Eleven calendar weeks elapsed; the
table records what shipped, what was substituted, and what was
de-scoped.

\small
\begin{longtable}{@{}p{1.15cm}p{3.0cm}p{8.0cm}p{2.4cm}@{}}
\toprule
\textbf{Week} & \textbf{Plan} & \textbf{Actual} & \textbf{Status} \\
\midrule
\endfirsthead
\toprule
\textbf{Week} & \textbf{Plan} & \textbf{Actual} & \textbf{Status} \\
\midrule
\endhead
W1 & EDA & EDA on the QA v2.1 validation 5k-row sample
     (\Cref{sec:dataset}) & shipped \\
W2 & \bmtwofive{} baseline & Full 8.8M-corpus \bmtwofive{} retrieve
     +  MRR/Recall (\Cref{sec:w2-results}) & shipped \\
W3 & RAG baseline (T5/BART/Phi + top-3) & T5-small RAG generator on
     the W2 top-3, 200-query sample baseline + the load-bearing
     paired full-dev comparison (\Cref{sec:gen-x-retrieval}) & shipped \\
W4 & Dense retrieval, ideally ANCE & Generic \texttt{all-MiniLM-L6-v2}
     on a 50k qrels-anchored sample with FAISS \texttt{IndexFlatIP}
     (\Cref{sec:w4-results}); ANCE / \msmarco{}-tuned encoders
     reserved for W4-B & shipped (smaller scope) \\
W5 & Cross-encoder rerank & \texttt{ms-marco-MiniLM-L-6-v2} over
     dense top-100, full-dev (\Cref{sec:w5-dense-rerank}) +
     \bmtwofive{}-top-100 arm (\Cref{sec:w5a}) & shipped \\
W6 & Generator fine-tuning & \emph{Substituted}: evaluation layer
     (\bertscore{} proxy + failure taxonomy + decode-budget closure;
     \Cref{sec:w6}).  The W3 paired comparison made evaluation
     calibration the higher-leverage axis than SFT, and the W6
     closure + W7 grounding together explain why SFT is unlikely
     to be the bottleneck. & substituted \\
W7 & Hallucination \& citation & \emph{Substituted}: grounding audit
...
W8 & Long-document handling & \emph{De-scoped} due to the project's
     2026-05-22 deadline and the choice to prioritise the W6/W7
...
W9 & Integration \& acceleration (ONNX, quantisation) & Engineering
...
W10 & Final evaluation & The headline paired comparison serves as
     the final evaluation; no CodaLab submission. & redirected \\
W11 & Research paper & This report. & shipped \\
W12 & Presentation & Out of report scope. & --- \\
\bottomrule
...
The two W6 / W7 substitutions are the project's main methodological
deviation from the plan.  The reasoning was empirical: once the W3
paired comparison shipped, evaluation calibration --- ``do these
deltas hold up under a semantic-similarity scorer?'', ``where do
the remaining regressions come from?'' --- gave a clearer story
than SFT would have, and the W6 closure run then made that story
\emph{actionable} by ruling out the most plausible cheap
intervention (the decoding budget).  The W7 grounding audit then
...
\paragraph{W4 / W5 numbers are on a qrels-anchored sample.}  The W4
...
W2 full-corpus number.  This caveat is propagated to the W5 dense+
rerank numbers, which inherit the same sampling.

\paragraph{\bmtwofive{} tokenizer mismatch with Anserini.}  The W2
...
W6 \bertscore{} proxy (\Cref{sec:w6-bertscore}) and the W7
...
\paragraph{The W7-A NLI sign flip is unresolved.}  Section
...
\paragraph{CPU-only.}  All numbers in this report were produced on
a six-core Apple Silicon laptop.  No GPU.  This is by design --- the
single-machine constraint is what makes the project's reproducibility
claim (one command per week regenerates every number) tractable ---
but it caps the kinds of generators that can be evaluated within
the deadline.
...
plausible from the W3 surface-form numbers alone are rejected on
...
The author thanks the project mentor, Tyler, for guidance on
problem framing and methodology throughout the internship period.
...
\texttt{ir\_datasets} loader~\cite{macavaney2021irdatasets}, the T5
and BART pretrained models~\cite{raffel2020t5,lewis2020bart}, the
...
\subsection{One config, one runner per week}

All four W2--W5 runners are driven by a single configuration file
at \texttt{configs/baseline.yaml}; the W6/W7 analysis scripts read
the on-disk W3 paired predictions directly.  \Cref{tab:repro-cmds}
records the canonical command for each stage.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{l>{\ttfamily\raggedright\arraybackslash}X}
    ...
    Stage & \normalfont Command \\
    \midrule
    W2  & python experiments/run\_retrieval.py \\
    W3  & python experiments/run\_generation\_baseline.py \\
    W4  & python experiments/run\_dense\_retrieval.py \\
    W5  & python experiments/run\_reranker.py \\
    W3 paired    & python scripts/run\_full\_generation\_and\_analysis.py \\
    W5-A         & python scripts/compare\_rerank\_first\_stages.py \\
    W6 BERTScore & python scripts/bertscore\_paired\_eval.py \\
    W6 taxonomy  & python scripts/regression\_failure\_taxonomy.py \\
    W6 closure   & python scripts/run\_full\_generation\_and\_analysis.py \mbox{--max-new-tokens}~128 \mbox{--out-suffix}~\_mnt128 \\
    W6-A/B/C     & scripts/tag\_query\_forms.py \ /\ analyze\_rerank\_by\_query\_form.py \ /\ regression\_query\_profile.py \\
    W7 grounding & python scripts/grounding\_audit.py \\
    W7-A NLI     & same script with \mbox{--nli-n-pairs}~3000 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
Every runner writes \texttt{outputs/<week>/manifest.json} alongside
...
Seed 42 is used everywhere a random choice enters: the W4
qrels-anchored distractor sample, the W6 BERTScore subsample
selection, the W6 regression-bucket triage sample, the W7-D
...
help and in the per-week reports' ``Reproduce'' sections.

\subsection{What is and isn't committed}

The project's \texttt{CLAUDE.md} codifies the policy:
\texttt{outputs/}, \texttt{data/}, and \texttt{.claude/} are
gitignored; the only experimental artefacts that ever enter git
are source code (under \texttt{src/}, \texttt{experiments/},
\texttt{scripts/}, \texttt{tests/}), \texttt{README.md}, the report
\emph{templates} under \texttt{reports/templates/}, and the final
\emph{viewable} report PDFs under \texttt{reports/generated/}.
This keeps the repository small and decouples regenerable
artefacts (every \texttt{metrics.json}, every \texttt{run.tsv})
from the source of truth.

\subsection{Engineering scaffolding}

\begin{itemize}
  \item \textbf{Tests.}  78 unit tests under \texttt{tests/}; runs
        in $<\!10$~s on the laptop; no network, no heavy deps.
        Slow tests gated behind \texttt{@pytest.mark.slow}.
  \item \textbf{Lint.}  \texttt{ruff} with the \texttt{F} and
        \texttt{W} rule sets enabled (pyflakes + whitespace); style
        rules deliberately disabled for the first pass.
  \item \textbf{CI.}  \texttt{.github/workflows/ci.yml} runs pytest
        + ruff on every push / PR to \texttt{main}.  Does not
        download \msmarco{} data or run the slow tests.
  \item \textbf{Lockfile.}  \texttt{requirements-lock.txt} is a
        pip-freeze snapshot of the author's macOS CPU-only dev
        environment.  Hash pinning and transitive closure are
        explicitly listed as TODO; the lockfile is calibrated for
        ``can I rebuild this on a comparable machine?'', not
        ``bit-identical builds on arbitrary infrastructure''.
\end{itemize}

\section{Full bucket counts and W6 analysis tables}
\label{sec:appendix-buckets}

\Cref{tab:buckets-full} reports the five-way bucket counts on the
full \fdev{} W3 paired comparison.  The classification is from
...
\begin{table}[H]
  \centering
  \caption{Five-way bucket counts on the full \fdev{} W3 paired
...
\hbadness=99999
\begin{itemize}
  \item \textbf{W2:} \bmtwofive{} retrieval, full corpus.  Output:
        \texttt{outputs/week02\_bm25/}.
  \item \textbf{W3:} RAG generation baseline (T5-small over top-3).
        Output:
        \code{outputs/week03\_\allowbreak generation\_\allowbreak \{bm25,reranked\}\_\allowbreak full/}.
  \item \textbf{W4:} dense retrieval, 50k qrels-anchored sample.
        Output: \texttt{outputs/week04\_dense/}.
  \item \textbf{W5:} cross-encoder rerank over dense top-100.
        Output: \texttt{outputs/week05\_reranker\_full/}.
  \item \textbf{W5-A:} same reranker over \bmtwofive{} top-100.
        Output:
        \code{outputs/week05\_\allowbreak rerank\_\allowbreak first\_\allowbreak stage\_\allowbreak compare/}.
  \item \textbf{W6:} \bertscore{} proxy + regression failure
        taxonomy + decode-budget closure.  Outputs:
        \code{outputs/week06\_\allowbreak bertscore\_\allowbreak proxy/},
        \code{outputs/week06\_\allowbreak analysis/},
        \code{outputs/week06\_\allowbreak taxonomy\_\allowbreak mnt128/}.
  \item \textbf{W6-A/B/C:} question-form tagging,
        per-form $\Delta$, regression structural profile.
        Outputs: \code{outputs/week06\_\allowbreak querytype/},
        \code{outputs/week06\_\allowbreak rerank\_\allowbreak by\_\allowbreak form/},
        \code{outputs/week06\_\allowbreak regression\_\allowbreak features/}.
  \item \textbf{W7:} lexical + 3-gram grounding audit; W7-A adds
        the NLI proxy; W7-C grounding $\leftrightarrow$ downstream
        correlation; W7-D low-grounding case study.  Outputs:
        \code{outputs/week07\_\allowbreak grounding/},
        \code{outputs/week07\_\allowbreak grounding\_\allowbreak correlation/},
        \code{outputs/week07\_\allowbreak low\_\allowbreak grounding\_\allowbreak cases/}.
\end{itemize}
\endgroup
