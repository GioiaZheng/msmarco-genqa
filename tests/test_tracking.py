...
def test_jsonl_tracker_writes_params_metrics_and_artifact(tmp_path):
    with ExperimentTracker(backend="jsonl", output_dir=tmp_path, run_name="smoke") as tracker:
    ...
    assert [row["kind"] for row in rows] == ["params", "metrics", "artifact"]
    assert rows[0]["payload"]["model"] == "t5-small"
    assert rows[1]["payload"]["metrics"]["token_f1"] == 0.37
    assert rows[1]["payload"]["step"] == 2
